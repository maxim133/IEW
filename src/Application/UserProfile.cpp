...
User::User(uint64_t uid, uint64_t sid, const std::string& FirstName, const std::string& LastName,
           const std::string& email) : uid(uid), sid(sid), FirstName(FirstName), 
           LastName(LastName), email(email)
{
    Validity = Valid;
}

User::User(const std::string& uid, const std::string& email) :
  email(email)
{
    this->uid = std::hash<std::string>{}(uid);
...
WaitingUser UsersBuffer::checkUser(const std::string& id, uint16_t SecretCode) 
{
    ...
    std::cout << "Получили код " << std::to_string(SecretCode) << std::endl;
    std::cout << "Ожидаем код " << std::to_string((*userIt).getSecretCode()) << std::endl;
